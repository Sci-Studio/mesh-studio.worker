...
namespace writer {
    ...
    bool SvgFileWriter::saveMeshToOutputFile(const geometry::Mesh& mesh) {
        ...
        out << "<svg xmlns=\"http://www.w3.org/2000/svg\" " << "viewBox=\"" << padMinX << " "
            << padMinY << " " << width << " " << height << "\" "
        ...
        for (auto triangle : mesh.triangles) {
            ...
            out << "    <polygon points=\"" << a.x << "," << a.y << " " << b.x << "," << b.y << " "
                << c.x << "," << c.y << "\" />\n";
        }
        out << "  </g>\n";
        ...
        for (auto edge : mesh.constraints) {
            if (edge.a < 0 || edge.b < 0 || edge.a >= static_cast<int>(mesh.points.size()) ||
